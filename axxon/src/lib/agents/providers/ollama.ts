...
const PLANNING_ANALYSIS_SYSTEM_PROMPT = [
  ...
  'For realtime monitoring, dashboard, graph, eval, tool-call, or agent-performance prompts, treat data exporter, realtime transport, telemetry scope, graphing stack, storage backend, retention window, and operational requirements as material planning slots.',
...
const PLANNING_ARTIFACT_SYSTEM_PROMPT = [
  ...
  'For realtime monitoring, dashboard, graph, eval, tool-call, or agent-performance prompts, explicitly name the exporter/collector, realtime transport, telemetry records, graphing approach, storage backend, retention window, and operational visibility.',
  'For monitoring plans, use this data-flow shape unless context says otherwise: producer telemetry -> exporter/collector -> storage or rollup layer -> backend API/WebSocket service -> dashboard client.',
  'Do not say that Next.js, the frontend, or the dashboard subscribes directly to an OpenTelemetry collector.',
  'Do not leave material implementation choices as alternatives like "Prometheus or InfluxDB"; choose the clarified option or ask before this stage.',
  'openQuestions must not include unresolved exporter, realtime transport, graphing, telemetry, storage, retention, access, security, or acceptance-test decisions.',
  ...
  'Do not invent stack choices such as React, Node, Express, PostgreSQL, or Chart.js as facts unless they are explicit, clarified, or already established by context; use assumptions or non-material notes for uncertain choices.',
