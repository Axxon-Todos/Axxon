...
function getValueForScope(item: AnalyticsLabelBarItem, scope: ScopeMode) {
...
export default function AnalyticsLabelBarChart({
  ...
  const visibleItems = items.filter((item) => getValueForScope(item, scope) > 0);
  ...
  const maxValue = Math.max(...visibleItems.map((item) => getValueForScope(item, scope)), 1);
