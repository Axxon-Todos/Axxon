...
export default function DashboardOverview() {
  ...
  const isLoading = isUserLoading || isBoardsLoading || isTodosLoading
  const hasError = userError || boardsError || todosError
    ...
    return (
      <div className="mx-auto max-w-[1480px]">
        <section className="glass-panel-strong rounded-[2rem] p-8">
          ...
          <p className="mt-3 max-w-2xl app-text-muted">
            The dashboard could not fetch your board or todo data. Refresh the page and try again.
          </p>
        </section>
      </div>
    )
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="app-badge">{items.length}</span>
      </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs app-text-muted">
                    {item.boardName && <span className="app-badge">{item.boardName}</span>}
                    {item.dueDate && <span className="app-badge">{dayjs(item.dueDate).format('MMM D')}</span>}
                  </div>
                </div>
              </div>
            </Link>
